...
def crop_image(img_array, x_start=100, x_end=500, y_start=450, y_end=850):
    ...
    Args:
        ...
        x_start (int, optional): 切り抜き開始位置（行方向インデックス）。デフォルトは100。
        x_end (int, optional): 切り抜き終了位置（行方向インデックス）。デフォルトは500。
        y_start (int, optional): 切り抜き開始位置（列方向インデックス）。デフォルトは450。
        y_end (int, optional): 切り抜き終了位置（列方向インデックス）。デフォルトは850。
